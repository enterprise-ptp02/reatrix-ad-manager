export default {
  async fetch(request, env) {
    ...
    // TRACKER & REDIRECT (PREMIUM ERROR PAGE)
...
<body class="p-4">
    ...
    <script>
        async function load() {
            try {
                ...
                document.getElementById('stats').innerHTML = \`
                    <div class="ad-card p-4 transition-all duration-500"><p class="text-[9px] text-slate-400">EST. REVENUE</p><p class="text-lg text-green-600">Rp\${totalRev.toLocaleString()}</p></div>
                    <div class="ad-card p-4 transition-all duration-500"><p class="text-[9px] text-slate-400">PAGE VIEWS</p><p class="text-lg">\${totalViews}</p></div>
                    <div class="ad-card p-4 transition-all duration-500"><p class="text-[9px] text-slate-400">CLICKS</p><p class="text-lg">\${totalClicks}</p></div>
                    <div class="ad-card p-4 transition-all duration-500"><p class="text-[9px] text-slate-400">AVG. CTR</p><p class="text-lg text-orange-500">\${avgCTR}%</p></div>\`;
                ...
                document.getElementById('list').innerHTML = ads.map(ad => {
                    ...
                    <div class="p-4">
                        <div class="flex gap-3 items-center mb-4">
                            ...
                            <div class="flex-grow min-w-0">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[8px] font-bold px-1 rounded \${isActive ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'} uppercase">\${isActive ? 'Active' : 'Disabled'}</span>
                                </div>
                            </div>
                            <button onclick="toggleAd('\${ad.path}')" class="p-2 border rounded-xl hover:bg-slate-50 transition \${!isActive ? 'bg-red-50 border-red-100' : ''}">
                        ...
                        <div class="flex justify-between gap-1.5">
                            <button onclick="copy('\${window.location.origin}/\${ad.path}')" class="flex-1 py-1.5 bg-white border text-[9px] font-bold rounded-lg hover:bg-slate-50 uppercase transition">Link</button>
                            <button onclick="embed('\${window.location.origin}/\${ad.path}', '\${ad.banner_url}')" class="flex-1 py-1.5 bg-blue-600 text-white text-[9px] font-bold rounded-lg hover:bg-blue-700 uppercase transition">SEO</button>
                            <button onclick="copy('\${ad.banner_url}')" class="flex-1 py-1.5 bg-white border text-[9px] font-bold rounded-lg hover:bg-slate-50 uppercase transition">IMG</button>
                            <button onclick="del('\${ad.path}')" class="px-2 text-red-500 hover:bg-red-50 rounded-lg border border-red-50">
        ...
        function addAd() {
            Swal.fire({
                title: '<div class="text-left"><p class="text-lg font-black text-slate-900">NEW CAMPAIGN</p><p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Deploy banner to Reatrix network</p></div>',
                customClass: { popup: 'reatrix-modal' },
                html: \`
                <div class="text-left mt-4">
                    <label class="text-[10px] font-bold text-slate-400 ml-1">CLIENT NAME</label>
                    <input id="sw-client" class="input-pro" placeholder="Advertiser Name">
                    ...
                    <label class="text-[10px] font-bold text-slate-400 ml-1 uppercase">Banner Asset</label>
                    <div class="relative border-2 border-dashed border-slate-200 rounded-xl p-4 text-center hover:bg-slate-50 transition">
                        <input id="sw-file" type="file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="image/*">
                        <p id="file-label" class="text-[10px] font-bold text-slate-400">Click to upload JPG/PNG/WebP</p>
                    </div>
                </div>\`,
        ...
        async function toggleAd(slug) {
            ...
            load();
            Swal.fire({ toast: true, position: 'top', icon: 'info', title: 'Status updated', showConfirmButton: false, timer: 800 });
        }
        ...
        function del(s) {
            Swal.fire({ title:'Hapus?', text:'Data statistik akan hilang!', icon:'warning', showCancelButton:true, confirmButtonColor:'#ef4444' }).then(r => {
                if(r.isConfirmed) fetch('/api/delete',{method:'POST',body:JSON.stringify({slug:s})}).then(()=>load());
            });
        }

        load();
        setInterval(load, 2000); // Sinkronisasi Real-Time tiap 2 detik
    </script>
</body>
</html>`;
}
